...
```bash
grep -r "host" config/*.conf > host_params.tx
```
...
```bash
zip -r project_backup.zip find config -name "*.сonf" find -name "*.log" errors.txt
```
...
```bash
grep -r -v '^$' archived/app.logs  > cleaned_app.logs 
```
